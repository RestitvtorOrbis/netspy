...
8. Enables HostModel options needed by inventory: native, other, symbols; maps `NetcoreApp3CompatMode` to `BundleOptions.BundleAllContent`; and enables compression only when the original v6 bundle has compressed entries.
...
Unknown raw file-type preservation is limited by HostModel's type inference. The MVP refuses rebuild when an unknown raw type is present rather than silently changing it. It also refuses bundles with duplicate assembly identity resolution ambiguities, R2R dirty entries, NativeAOT, non-Windows hosts, or non-x64 Windows hosts.
...
Acceptance: HostModel receives exactly one host `FileSpec` whose `SourcePath` is the reconstructed host and whose `BundleRelativePath` equals the `hostName` constructor argument; a missing, duplicate, or mismatched host fails before generation. It generates uncompressed/compressed and .NET 5 compatibility-mode bundles with one/multiple replacements; parser flags prove compatibility mode maps to `BundleOptions.BundleAllContent`; no bundle relative path is used as a disk extraction path.
...
| BND-017 | PR-05 | Planned | — | — |
| BND-018 | PR-05 | Planned | — | — |
| BND-019 | PR-05 | Planned | — | — |
| BND-020 | PR-06 | Planned | — | — |
| BND-021 | PR-06 | Planned | — | — |
| BND-022 | PR-06 | Planned | — | — |
| BND-023 | PR-06 | Planned | — | — |
| BND-024 | PR-06 | Planned | — | — |
| BND-025 | PR-06 | Planned | — | — |
...
Every row is mandatory before that ticket is reviewed. Test class names are contracts established by the ticket; implementations must use these names so commands do not drift. The “normal build” command for BND-001 through BND-027 is exactly `pwsh -NoProfile -File ./build.ps1`; BND-028 runs its four modes separately as shown. On this Ubuntu host, every normal-build cell has the same explicit blocker: `pwsh` and Windows/.NET Framework COM/WPF build support are absent, so the command must run on Windows CI. Linux runs cross-platform net10 scoped commands; historical-generation PowerShell, `net10.0-windows`, net48, and execution rows are also explicitly blocked here and must not be claimed locally.
...
The current environment has .NET SDK `10.0.111` and no `pwsh`/standalone `msbuild`.
