...
| NSPY-004 | approved | `test(NSPY-004): lock netSpy branding contracts` | Independently reviewed focused coverage for constants/compatibility identity, assembly metadata, neutral resources/upstream provenance, IPC source seams, visual/template/accessibility structure, six required 32-bit ICO frames, and README identity. Product and integration-test projects build for `net10.0-windows` with 0 warnings/errors; filtered execution is blocked on Linux by the unavailable `Microsoft.WindowsDesktop.App 10.0.0` runtime and is not claimed. README/source, protected-hash, and whitespace checks pass; Windows UI smoke acceptance remains outstanding. |
